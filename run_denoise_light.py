...
import numpy as np
import pandas as pd
import os, sys, pdb, argparse, copy, scipy, jinja2, warnings
...
parser.add_argument('--out_file_append',
                    help='appendix for output saved file')
...
out_file_append = args.out_file_append

def denoise(img_file, tsv_file, out_path, col_names=False, hp_filter=False, lp_filter=False, out_figure_path=False, out_file_append=False):
    nii_ext = '.nii.gz'
    print(out_file_append)
    FD_thr = [.5]
    sc_range = np.arange(-1, 3)
    constant = 'constant'
    six_mps = ['trans_x', 'trans_y', 'trans_z', 'rot_x', 'rot_y', 'rot_z']
    friston_cols = ['trans_x', 'trans_y', 'trans_z', 'rot_x', 'rot_y', 'rot_z', \
                    'trans_x_power2',    'trans_y_power2',      'trans_z_power2', \
                    'rot_x_power2',      'rot_y_power2',        'rot_z_power2', \
                    'trans_x_pre',       'trans_y_pre',         'trans_z_pre', \
                    'rot_x_pre',         'rot_y_pre',           'rot_z_pre', \
                    'trans_x_pre_power2','trans_y_pre_power2',  'trans_z_pre_power2', \
                    'rot_x_pre_power2',  'rot_y_pre_power2',    'rot_z_pre_power2' ]
    ...
    if not out_file_append:
        out_file_append = '_NR' + nii_ext
    out_file_append_html = out_file_append[0: out_file_append.find(nii_ext)]

    save_img_file = pjoin(out_path, file_base + out_file_append)
    data = img.get_fdata()
    df_orig = pd.read_csv(tsv_file, sep='\t', na_values='n/a')
    ...
    if col_names:
        if 'friston24' in col_names or 'Friston24' in col_names:
            # 6 MPs & 6 MPs Squared
            if 'friston24' in col_names:
                col_names.remove('friston24')
            if 'Friston24' in col_names:
                col_names.remove('Friston24')
            for col in friston_cols:
                col_names.append(col)
            print(col_names)
            col_names = list(set(col_names))
            # add in 6 MPs prior timepoint
            for mp in six_mps:
                df[mp + '_pre'] = np.hstack((np.array(df[mp].iloc[1:]), \
                                                    np.array(df[mp].iloc[-1])))
                df[mp + '_pre_power2'] = np.power(df[mp + '_pre'], 2)
    ...
    png_append = '_' + img_name[0:img_name.find('.')] + out_file_append_html + '.png'
        ...
        for thr_idx, thr in enumerate(FD_thr):
            ...
            for iidx in np.where(idx)[0]:
                for buffer in sc_range:
                    ...
                    if curr_idx >= 0 and curr_idx <= len(idx)-1:
    ...
    # pdb.set_trace()
    now = datetime.now()
    datestr = now.strftime('%H') + '_' + now.strftime('%M') + '_' + now.strftime('%S') + '_' + now.strftime('%f')
    mask_file = pjoin(out_figure_path, 'mask_temp_' + datestr + '.nii.gz')
    ...
    for in_file in img_files:
        ...
        try:
            computedvars = nac.ComputeDVARS(in_file=in_file, in_mask=mask_file)
            out = computedvars.run()

            df = pd.read_csv(out.outputs.out_std, sep='\t', header=None)
            temp = np.array(df[0])
            temp = np.hstack((np.mean(temp), temp))

            # temp = nac.compute_dvars(in_file=in_file, in_mask=mask_file)[1]
            dvars.append(np.hstack((temp.mean(), temp)))
            try:
                cmd = 'rm ' + out.outputs.out_std
                os.system(cmd)
            except:
                print(' not able to remove file: ')
                print(out.outputs.out_std)
                
            del temp, df, out, computedvars

        except:
            # pdb.set_trace()
            img = load_niimg(img_file)
            data = np.reshape(img.get_fdata(), (-1, Ntrs))
            # data_m = data[nifti_masker.mask_img_.get_data(), :]
            data_m = data[np.ndarray.flatten(nifti_masker.mask_img_.get_fdata()==1), :]
            dvars_curr = np.sqrt(np.mean(np.square(np.diff(data_m, n=1, axis=1)), axis=0))
            dvars.append(dvars_curr)
            # img_mask = im #nifti_masker.mask_img_
    ...
    # SKIP T-stat maps for nuisance regressors
    # mx = []
    ...
    TEMPLATE_FILE = pjoin(os.getcwd(), "report_template_light.html")
    template = templateEnv.get_template(TEMPLATE_FILE)

    outputText = template.render(templateVars)

    html_file = pjoin(out_figure_path, img_name[0:img_name.find('.')] + out_file_append_html + '.html')
...
denoise(img_file, tsv_file, out_path, col_names, hp_filter, lp_filter, out_figure_path, out_file_append)
